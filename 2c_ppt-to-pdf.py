...
def is_ppt_file(file_path):
    ...
    # Check if the file has a .ppt or .pptx extension
    if file_extension in ['.ppt', '.pptx', '']:
        with open(file_path, 'rb') as f:
            # Read the first few bytes to determine the file type
            magic_number = f.read(4)
            # Check if it matches the magic numbers for PPT or PPTX
            if file_extension == '.ppt' or file_extension == '':
                if magic_number == b'\xD0\xCF\x11\xE0':  # Check for PPT magic number
                    return True
            if file_extension == '.pptx' or file_extension == '':
                if magic_number == b'\x50\x4B\x03\x04':  # Check for PPTX magic number
                    return True

    return False  # If the file is not a recognized PPT file

def open_file_with_timeout(ppt, file_path, timeout=10):
    start_time = time.time()
    document = None
    
    while True:
        try:
            document = ppt.Presentations.Open(file_path)
...
def convert_ppt_to_pdf(input_folder):
    # Create COM object for PowerPoint application
        ...
        for filename in os.listdir(folder):
            ...
            if os.path.isdir(file_path):
                ...
                if is_ppt_file(file_path):
                    ...
                    output_filename = f"{file_name}_PowerPoint.pdf"
                    ...
                    try:
                        ...
                        doc = open_file_with_timeout(ppt, file_path)
                        if doc is None:
                            continue

                        # Set output filename - redundant?
                        #output_path = os.path.join(folder, output_filename)

                        # Save as PDF
                        doc.SaveAs(output_path, 32) # 32 is for PDF format
...
convert_ppt_to_pdf(input_folder)
